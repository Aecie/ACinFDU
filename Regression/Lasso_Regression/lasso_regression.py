"""
Lasso regression introduces an L1 regulation alpha*||w|| in linear least squares

The objective function is ||y - XW||^2 + alpha*||W||

This model is optimizing the same objective function as the Elastic Net 

[REF] https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.Lasso.html#sklearn.linear_model.Lasso
"""
...
from cuml.linear_model import Lasso as CudaLasso
...
from sklearn.linear_model import Lasso as CPULasso
...
def plot_triple_alphas_effect(train_X, train_Y, use_polynomial: bool, alpha_list: list, style_list: list, use_cuda: bool):
    ...
    for alpha, styple in zip(alpha_list, style_list):
        model = CudaLasso(alpha) if use_cuda else CPULasso(alpha)
        pipeline_params = []
        if use_polynomial:
